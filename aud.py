...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        if predicted_letter != last_detected_letter:
            ...
            image_path = os.path.join(image_folder, f"{predicted_letter}.jpg")
            if os.path.exists(image_path):
                image = cv2.imread(image_path)
                # Resize the displayed image to fit the region
                image = cv2.resize(image, (160, 160))

                # Add the image to the deque
                detected_images.append(image)

        # Display the last three detected images horizontally at the bottom
        x_offset = 10
        for img in detected_images:
            img_height, img_width, _ = img.shape
            frame[H - 160 : H, x_offset : x_offset + img_width] = img
            x_offset += img_width
